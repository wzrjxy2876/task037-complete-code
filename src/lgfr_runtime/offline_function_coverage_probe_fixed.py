...
def best_coverage_swap(
    ...
    """Find the best equal-budget swap without repeatedly recomputing coverage.

    The original implementation evaluated every candidate pair by rebuilding
    the full retained set and calculating a U x |K| maximum.  For a 3072-unit
    FFN layer this results in tens of billions of comparisons per swap.

    This implementation computes the best and second-best retained
    representative for every query unit once.  Removing boundary unit b then
    only requires choosing top2 where b was top1, and restoring r requires one
    elementwise maximum with similarity[:, r].
    """
    ...
    retained_units = np.flatnonzero(~removed_mask)
    retained_similarity = similarity[:, retained_units]  # [U,K]

    # Top-1 and top-2 retained representatives for every query unit.
    if retained_similarity.shape[1] == 1:
        top1_local = np.zeros(similarity.shape[0], dtype=np.int64)
        top1_value = retained_similarity[:, 0].astype(np.float64)
        top2_value = np.zeros_like(top1_value)
    else:
        top2_local = np.argpartition(
            retained_similarity,
            kth=retained_similarity.shape[1] - 2,
            axis=1,
        )[:, -2:]
        top2_candidates = np.take_along_axis(
            retained_similarity,
            top2_local,
            axis=1,
        )
        order = np.argsort(top2_candidates, axis=1)
        second_local = np.take_along_axis(
            top2_local,
            order[:, :1],
            axis=1,
        )[:, 0]
        top1_local = np.take_along_axis(
            top2_local,
            order[:, 1:],
            axis=1,
        )[:, 0]
        top1_value = retained_similarity[
            np.arange(similarity.shape[0]), top1_local
        ].astype(np.float64)
        top2_value = retained_similarity[
            np.arange(similarity.shape[0]), second_local
        ].astype(np.float64)

    top1_unit = retained_units[top1_local]
    baseline_coverage = float(np.mean(top1_value))

    # Coverage after removing each boundary unit, before restoring a unit.
    # Shape: [U,B].
    base_without_boundary = np.repeat(
        top1_value[:, None],
        len(boundary_units),
        axis=1,
    )
    for boundary_pos, boundary in enumerate(boundary_units):
        affected = top1_unit == boundary
        base_without_boundary[affected, boundary_pos] = top2_value[affected]

    best = None

    for restored in removed_units:
        restored_similarity = similarity[:, restored].astype(np.float64)
        # Evaluate all boundary replacements for this restored unit at once.
        candidate_scores = np.maximum(
            base_without_boundary,
            restored_similarity[:, None],
        ).mean(axis=0)

        for boundary_pos, replacement in enumerate(boundary_units):
            score = float(candidate_scores[boundary_pos])
...
def main() -> None:
    ...
    for position, (layer, key) in enumerate(
        ...
        print(
            f"  similarity ready; starting optimized coverage repair "
            f"(max_swaps={args.max_swaps}, boundary_count={args.boundary_count})"
        )
