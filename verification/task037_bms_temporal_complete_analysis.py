...
def analyse(args):
    ...
    domsum=domains[domains.selected].rename(columns={"selected_primary_category":"category"}).merge(ranks.groupby(["domain_id","category"],as_index=False).agg(reversal_fraction=("reversal_fraction","mean"),strict_reversals=("strict_reversals","sum"),comparable_pairs=("comparable_pairs","sum")),on=["domain_id","category"],how="left"); domsum=domsum.merge(units.groupby("domain_id").size().rename("tested_size"),on="domain_id",how="left"); domsum.to_csv(out/"task_bms_temporal_domain_summary.csv",index=False)
