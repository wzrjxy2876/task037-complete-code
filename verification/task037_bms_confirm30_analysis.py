...
def analyse(a):
 ...
 # pilot comparison
 try:
  ps=json.load(open(Path(a.pilot_output)/'task_bms_temporal_summary.json')); rows=[{'study':'Discovery_Pilot','videos':3,'contexts_per_video':80,'domains':9,'units':36,'overall_reversal_fraction':ps.get('reversal_fraction'),'severity_matched_reversal_fraction':ps.get('severity_matched_reversal_fraction'),'same_span_reversal_fraction':ps.get('same_span_reversal_fraction')},{'study':'Independent_Confirmation','videos':30,'contexts_per_video':20,'domains':9,'units':36,'overall_reversal_fraction':float(rev.strict_reversal.mean()),'severity_matched_reversal_fraction':float(sev.strict_reversals.sum()/sev.comparable_pairs.sum()) if len(sev) else np.nan,'same_span_reversal_fraction':float(same.strict_reversals.sum()/same.comparable_pairs.sum()) if len(same) else np.nan}]
