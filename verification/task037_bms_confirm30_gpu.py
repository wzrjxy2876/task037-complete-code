...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Task037 independent 30-video confirmation")
    ...
    parser.add_argument("--context_manifest", default="", help="CSV with frozen span/canonical_pair_index subset")
    parser.add_argument(
        "--intervention_mode",
...
def run_probe(args: argparse.Namespace) -> dict[str, Any]:
    ...
    # Task037 authoritative global indices come from the frozen BMS mapping,
    # whose ordering is different from the probe's discovery offsets.
    auth_map_path = os.environ.get("TASK037_AUTH_MAPPING", "")
    if auth_map_path:
        with open(auth_map_path, newline="", encoding="utf-8") as _fh:
            _reader = csv.DictReader(_fh)
            _auth = {}
            for _row in _reader:
                _ut = "head" if "attention" in str(_row["unit_type"]).lower() else "neuron"
                _auth[(str(_row["layer"]), _ut, int(_row["unit_index"]))] = int(_row["global_index"])
        _remapped = []
        for _u in selected_units:
            _key = (str(_u.layer_name), str(_u.unit_type), int(_u.unit_index))
            if _key not in _auth:
                raise KeyError(f"authoritative BMS mapping missing {_key}")
            _remapped.append(SelectedUnit(global_index=_auth[_key], layer_name=_u.layer_name, unit_type=_u.unit_type, unit_index=_u.unit_index, spec=_u.spec))
        selected_units = _remapped
    ...
    # Confirmation uses the predeclared 20-context subset (indices 0,4,8,12 per span).
    if args.context_manifest:
        import pandas as _pd
        _ctx = _pd.read_csv(args.context_manifest)
        _allowed = {(int(r.span), int(r.canonical_pair_index)) for r in _ctx.itertuples()}
        if len(_allowed) != 20:
            raise AssertionError("confirmation context manifest must contain 20 unique contexts")
        interventions = [it for it in interventions if (int(it.block_size), int(it.pair_index)) in _allowed]
        if len(interventions) != 20:
            raise AssertionError("filtered intervention subset must contain 20 contexts")
